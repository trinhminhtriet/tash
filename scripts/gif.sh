...
TASH_PATH=$(dirname $(dirname $(realpath "$0")))
TASH_EXAMPLE_PATH="./test/playground"
OUTPUT_FILE="$TASH_PATH/res/output.json"
OUTPUT_GIF="$TASH_PATH/res/output.gif"

_init() {
  cd "$TASH_EXAMPLE_PATH"
...
_simulate_commands() {
  ...
  local CMD='
    ...
    sleep 0.5s
    _mock "tash run ping --tags remote"
    sleep 1.3s
    clear

    _mock "# Ping all hosts in parallel"
    sleep 0.5s
    _mock "tash run ping --tags remote --parallel"
    sleep 1.3s
    clear
    ...
    sleep 1s
    _mock "tash run info --all --output table"
    sleep 2s
    clear
  '
