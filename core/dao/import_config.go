...
// parseConfig is the starting point of reading and importing tash config files.
// The following summary represents the logic:
// 1. Initialize the root config and optionally, if it exists, add the user config (XDG_HOME_CONFIG/tash/config.yaml) to the imports list of the root config
...
func (c *ConfigYAML) parseConfig() (Config, error) {
	...
	cr.Envs = []string{
		fmt.Sprintf("TASH_DIR=%s", c.Dir),
		fmt.Sprintf("TASH_PATH=%s", c.Path),
	}
	...
	// Check if there's a user config file ($XDG_HOME_CONFIG/tash/config.yaml)
	if c.UserConfigFile != nil {
		cr.Imports = append(cr.Imports, Import{Path: *c.UserConfigFile, context: c.Path, contextLine: -1})
	}

	// Import tash configs and check cyclic dependencies
	n := Node{
		Path:    c.Path,
		Imports: cr.Imports,
	}
