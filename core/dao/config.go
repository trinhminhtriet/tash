...
var (
	ACCEPTABLE_FILE_NAMES = []string{"tash.yaml", "tash.yml", ".tash.yaml", ".tash.yml"}
...
func ReadConfig(configFilepath string, userConfigPath string, sshConfigFile string, noColor bool) (Config, error) {
	...
	if configFilepath != "" {
		...
		// check if env variable TASH_CONFIG is set, and if not found
		// return no config found
		filename, err := core.FindFileInParentDirs(wd, ACCEPTABLE_FILE_NAMES)
		if err != nil {
			val, present := os.LookupEnv("TASH_CONFIG")
			if present {
				filename = val
			} else {
				return Config{}, err
...
func getUserConfigFile(userConfigPath string) (*string, error) {
	...
	// Env
	val, present := os.LookupEnv("TASH_USER_CONFIG")
	if present {
	...
	defaultUserConfigPath := filepath.Join(defaultUserConfigDir, "tash", "config.yaml")
	if _, err := os.Stat(defaultUserConfigPath); err == nil {
		return &defaultUserConfigPath, nil
	}

	defaultUserConfigPath = filepath.Join(defaultUserConfigDir, "tash", "config.yml")
	if _, err := os.Stat(defaultUserConfigPath); err == nil {
		return &defaultUserConfigPath, nil
	}
...
func getSSHConfigPath(sshConfigPath string) (*string, error) {
	...
	// Env
	val, present := os.LookupEnv("TASH_SSH_CONFIG")
	if present {
		return &val, nil
	}
...
// Open tash config in editor and optionally go to line matching the task name
...
// Open tash config in editor and optionally go to line matching the server name
...
// Open tash config in editor and optionally go to line matching the target name
...
// Open tash config in editor and optionally go to line matching the spec name
...
func InitTash(args []string) ([]Server, error) {
	// Choose to initialize tash in a different directory
	// 1. absolute or
	// 2. relative or
	// 3. working directory
	...
	configPath := filepath.Join(configDir, "tash.yaml")
	if _, err := os.Stat(configPath); err == nil {
		return []Server{}, &core.AlreadyTashDirectory{Dir: configDir}
	}
...
`,
	...
	fmt.Println("\nInitialized tash in", configDir)
	fmt.Println("- Created tash.yaml")
