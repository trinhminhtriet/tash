...
func ParseServers(
	...
	for i := range *servers {
		...
		// that has ProxyJump, ProxyJump alias or if in tash it has a bastion: server-1
		//  1. proxyjump alias
		//	2. proxyjump
		//	3. bastion alias, in this case we need to handle multiple bastions
		// In-case tash has bastions defined, then skip resolving
...
func (run *Run) setKnownHostsFile(knownHostsFileFlag string) error {
	...
	value, found := os.LookupEnv("TASH_KNOWN_HOSTS_FILE")
	if found {
