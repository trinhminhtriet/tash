...
build-and-link:
	go build \
		...
		-a -tags netgo -o dist/${NAME} main.go
	cp ./dist/tash ~/.local/bin/tash

gen-man:
	go run -ldflags="-X 'github.com/trinhminhtriet/tash/cmd.buildMode=man' -X '${PACKAGE}/cmd.version=${VERSION}' -X '${PACKAGE}/cmd.commit=${GIT}' -X '${PACKAGE}/cmd.date=${DATE}'" ./main.go gen-docs
